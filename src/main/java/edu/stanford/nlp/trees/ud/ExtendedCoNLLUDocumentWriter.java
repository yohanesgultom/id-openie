...
public class ExtendedCoNLLUDocumentWriter extends CoNLLUDocumentWriter {
    ...
    public String printSemanticGraph(SemanticGraph sg, boolean unescapeParenthesis) {
        ...
        for (IndexedWord token : sg.vertexListSorted()) {
            ...
            String misc = token.getString(CoreAnnotations.NamedEntityTagAnnotation.class, "_");
            misc = misc.equalsIgnoreCase("O") ? "_" : misc;
            ...
            sb.append(String.format("%d\t%s\t%s\t%s\t%s\t%s\t%d\t%s\t%s\t%s%n", token.index(), word,
                    lemma, pos, upos, featuresString, govIdx, relnName, additionalDepsString, misc));
        }
        sb.append("\n");
